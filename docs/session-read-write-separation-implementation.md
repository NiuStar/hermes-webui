...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D02、D03、D04（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D05（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D02、D03、D09、D14、D15（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D01、D06、D07、D08（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D10、D11、D12、D13（含验证及回退）。
...
SSE现有run_id:seq保持兼容；新客户端协商projection_v1，附加session_seq及anchor_id，不能强改旧Last-Event-ID或after_event_id/after_seq的含义；新增session_seq仅属于协商后的projection_v1，不能把既有SSE RFC的run局部游标描述成已实现会话全局序号。GET返回本页实际已交付的resume_seq，同时独立返回快照upper_seq；has_more时不能把游标直接推进upper_seq。SSE从持久events补齐，通知只作唤醒，按protocol-v2处理无遗漏交接，以event_key幂等。归档不是删除事件，切换历史generation后重新以covered_seq裁掉已归档增量。过期epoch游标返回明确409/reload_required，不将旧偏移用于新版本。旧客户端继续旧GET/SSE，服务端不能悄悄替换返回语义。

## 6. worker与非追加操作

风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D14、D15、D16、D17（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D05、D19、D22、D23、D26（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D04、D20、D21、D24、D25（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D01（含验证及回退）。
...
风险与处理：见 [逐项风险矩阵](session-read-write-separation-risk-matrix.md) D26（含验证及回退）。
