...
| Run journal / replay | Rebuilds emitted runtime events after reconnect or restart | Must be cursor-safe and idempotent; unfinished runs remain durable while closed runs may use bounded retention with an explicit session-snapshot fallback | Duplicate assistant text, thinking text, tool cards, or compression cards; grow without bound by retaining every closed run forever |
