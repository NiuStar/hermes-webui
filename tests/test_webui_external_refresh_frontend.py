...
def test_same_session_force_reload_keeps_loaded_transcript_width_hint():
    ...
    # The width hint is always clamped to the server ceiling and the request
    # always carries msg_limit. Older history remains reachable through bounded
    # msg_before pages; a bare full-transcript fallback can OOM the server.
    assert "const boundedReloadLimit = Math.max(1, Math.min(reloadLimit || _INITIAL_MSG_LIMIT, _msgLimitMax));" in SESSIONS_JS
    assert "&msg_limit=${boundedReloadLimit}" in SESSIONS_JS
